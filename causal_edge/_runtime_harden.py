...
2026-04-18: manual ``causal-edge paper`` hung 5h13m with the same
...
statement of ``causal_edge/cli.py`` — before even ``click`` is imported.
"""
...
def install_from_env() -> None:
    """Install trap + opt-in timeout from ``CAUSAL_EDGE_TIMEOUT_SECONDS`` env.

    One-call convenience for CLI entry. Idempotent. ``CAUSAL_EDGE_TIMEOUT_SECONDS``
    value must be a positive integer to enable the wall-clock timeout.
    """
    install_tree_kill_trap()
    raw = os.environ.get("CAUSAL_EDGE_TIMEOUT_SECONDS", "").strip()
    if raw.isdigit():
